...
pub fn digest(data: &[u8], key: [u8; 16]) -> u64 {
...
pub fn digest_hex(data: &[u8], key: [u8; 16]) -> String {
    format!("{:016x}", digest(data, key))
}
...
mod tests {
    ...
    fn siphash_2_4_reference_vectors() {
        ...
        for (i, expected) in vectors.iter().enumerate() {
            ...
            assert_eq!(digest(&msg, key), expected_u64);
            msg.push(i as u8);
        }
    }
    ...
    fn incremental_update_matches_one_shot() {
        ...
        let data = b"moonlight-siphash";
        let mut hasher = SipHash24::new(key);
        hasher.update(&data[..7]);
        hasher.update(&data[7..]);
        assert_eq!(hasher.finalize(), digest(data, key));
    }
}
