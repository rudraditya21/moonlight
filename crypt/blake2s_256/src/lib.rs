...
#[derive(Debug, Clone)]
pub struct Blake2s<const OUT: usize> {
    state: [u32; 8],
    buffer: [u8; 64],
    buffer_len: usize,
    t0: u32,
...
impl<const OUT: usize> Blake2s<OUT> {
    pub fn new() -> Self {
        let mut state = IV;
        let param = 0x01010000u32 ^ (OUT as u32);
        state[0] ^= param;
        Blake2s {
            state,
            buffer: [0u8; 64],
...
impl<const OUT: usize> Default for Blake2s<OUT> {
    fn default() -> Self {
        Blake2s::new()
    }
}
...
pub type Blake2sHasher = Blake2s<OUT>;
