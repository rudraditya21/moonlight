# MS_NRTP Protocol

## Location
`network/proto/src/ms_nrtp/`

## Overview
This module provides a full MS_NRTP protocol implementation intended for use by Moonlight modules. Use the public types and functions below as the stable entry points.

## Public Types
### Structs
- `AsyncNrtpClient`
- `AsyncNrtpServer`
- `NrtpClient`
- `NrtpClientConfig`
- `NrtpMessage`
- `NrtpServer`
- `NrtpServerConfig`

### Enums
- `NrtpMessageType`

### Constants
- `MSNRTP_DEFAULT_PORT`

## Key Entry Points
### Clients
- `AsyncNrtpClient`
- `NrtpClient`
- `NrtpClientConfig`

### Servers
- `AsyncNrtpServer`
- `NrtpServer`
- `NrtpServerConfig`

### Free Functions
- `pub fn resolve(&self, addr: SocketAddr, name: &str) -> CoreResult<Vec<SocketAddr>> {`
- `pub fn unregister(&self, addr: SocketAddr, name: &str) -> CoreResult<()> {`

### Methods
- `AsyncNrtpClient`
  - `pub async fn connect() -> CoreResult<Self>`
  - `pub async fn resolve(&self, addr: SocketAddr, name: &str) -> CoreResult<Vec<SocketAddr>>`
- `AsyncNrtpServer`
  - `pub async fn bind(addr: SocketAddr, _config: NrtpServerConfig) -> CoreResult<Self>`
  - `pub async fn serve(&self) -> CoreResult<()>`
- `NrtpClient`
  - `pub fn connect(config: NrtpClientConfig) -> CoreResult<Self>`
  - `pub fn register( &self, addr: SocketAddr, name: &str, addresses: Vec<SocketAddr>, ) -> CoreResult<()>`
- `NrtpMessage`
  - `pub fn encode(&self) -> Vec<u8>`
  - `pub fn decode(data: &[u8]) -> CoreResult<Self>`
- `NrtpServer`
  - `pub fn bind(addr: SocketAddr, config: NrtpServerConfig) -> CoreResult<Self>`
  - `pub fn local_addr(&self) -> CoreResult<SocketAddr>`
  - `pub fn serve(&self) -> CoreResult<()>`

## Usage Notes
- Start with the client or server structs listed above, then follow their constructors and connect/bind/listen methods if present.
- Encode and decode helpers are exposed as public functions or methods in this module; use them to build protocol frames.
- When in doubt, search for the types above in the code to find concrete examples and tests in the same module directory.

## Tests and Examples
- Unit tests live alongside the implementation in `network/proto/src/ms_nrtp/`.
- Protocol‑level fuzz/negative tests are located in `network/proto/tests/negative_fuzz.rs` where applicable.

