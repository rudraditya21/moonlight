...
moonlight(auxiliary/crypto/hash_sha2_256)> show options
moonlight(auxiliary/crypto/hash_sha2_256)> set INPUT abc
moonlight(auxiliary/crypto/hash_sha2_256)> run
```

## Verify Example
```text
moonlight(auxiliary/crypto/hash_sha2_256)> set HASH <expected-hex>
moonlight(auxiliary/crypto/hash_sha2_256)> run
```

## Notes
- Output is lowercase hexadecimal.
- This module computes `SHA2-256` and optionally verifies against `HASH`.
