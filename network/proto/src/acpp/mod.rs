...
const XOR_KEY: [u8; 256] = [
    14, 57, 248, 5, 196, 1, 85, 79, 12, 172, 133, 125, 134, 138, 181, 23,
    62, 9, 200, 53, 244, 49, 101, 127, 60, 156, 181, 109, 150, 154, 165, 7,
    46, 25, 216, 37, 228, 33, 117, 111, 44, 140, 165, 157, 102, 106, 85, 247,
    222, 233, 40, 213, 20, 209, 133, 159, 220, 124, 85, 141, 118, 122, 69, 231,
    206, 249, 56, 197, 4, 193, 149, 143, 204, 108, 69, 189, 70, 74, 117, 215,
    254, 201, 8, 245, 52, 241, 165, 191, 252, 92, 117, 173, 86, 90, 101, 199,
    238, 217, 24, 229, 36, 225, 181, 175, 236, 76, 101, 221, 38, 42, 21, 183,
    158, 169, 104, 149, 84, 145, 197, 223, 156, 60, 21, 205, 54, 58, 5, 167,
    142, 185, 120, 133, 68, 129, 213, 207, 140, 44, 5, 253, 6, 10, 53, 151,
    190, 137, 72, 181, 116, 177, 229, 255, 188, 28, 53, 237, 22, 26, 37, 135,
    174, 153, 88, 165, 100, 161, 245, 239, 172, 12, 37, 29, 230, 234, 213, 119,
    94, 105, 168, 85, 148, 81, 5, 31, 92, 252, 213, 13, 246, 250, 197, 103,
    78, 121, 184, 69, 132, 65, 21, 15, 76, 236, 197, 61, 198, 202, 245, 87,
    126, 73, 136, 117, 180, 113, 37, 63, 124, 220, 245, 45, 214, 218, 229, 71,
    110, 89, 152, 101, 164, 97, 53, 47, 108, 204, 229, 93, 166, 170, 149, 55,
    30, 41, 232, 21, 212, 17, 69, 95, 28, 188, 149, 77, 182, 186, 133, 39,
];
    ...
    pub fn decode(data: &[u8], validate_checksums: bool) -> CoreResult<Self> {
        ...
        if validate_checksums {
            ...
            if expected_msg != read_message_checksum {
                return Err(CoreError::Parse("invalid acpp message checksum".to_string()));
            }
            if payload_size == 0xFFFFFFFF || payload.len() == payload_size as usize {
                ...
                if expected_payload != payload_checksum {
                    return Err(CoreError::Parse("invalid acpp payload checksum".to_string()));
                }
            }
        }
