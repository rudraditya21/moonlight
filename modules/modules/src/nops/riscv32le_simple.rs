...
pub struct NopRiscv32leSimpleFactory;

impl ModuleFactory for NopRiscv32leSimpleFactory {
    fn metadata(&self) -> &ModuleMetadata {
        static META: OnceLock<ModuleMetadata> = OnceLock::new();
        META.get_or_init(|| {
            ModuleMetadata::new(
                "nops/riscv32le/simple",
                "Simple RISC-V 32-bit NOP generator",
                ModuleCategory::Nop,
                "moonlight",
            )
...
mod tests {
    ...
    #[test]
    fn riscv32_simple_length() {
        let factory = NopRiscv32leSimpleFactory;
