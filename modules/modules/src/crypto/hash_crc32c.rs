...
pub struct HashCrc32cModule {
    base: ModuleBase,
}

impl HashCrc32cModule {
    pub fn new() -> Self {
...
impl Module for HashCrc32cModule {
    ...
    fn run(&mut self, _ctx: &ModuleContext) -> Result<ModuleResult, ModuleError> {
        ...
        let hash = crc32c::digest_hex(input.as_bytes());
        if expected.is_empty() {
            return Ok(ModuleResult::ok(&format!("crc32c: {}", hash)));
        }
...
pub struct HashCrc32cFactory;

impl ModuleFactory for HashCrc32cFactory {
    fn metadata(&self) -> &ModuleMetadata {
...
mod tests {
    ...
    #[test]
    fn crc32c_module_compute() {
        let mut module = HashCrc32cModule::new();
        module.options_mut().set("INPUT", "123456789").expect("set");
        let result = module.run(&ModuleContext { session_id: 1 }).expect("run");
        assert!(result.message.contains("e3069283"));
    }

    #[test]
    fn crc32c_module_verify() {
        let mut module = HashCrc32cModule::new();
        module.options_mut().set("INPUT", "123456789").expect("set");
        module.options_mut().set("HASH", "e3069283").expect("set");
        let result = module.run(&ModuleContext { session_id: 1 }).expect("run");
