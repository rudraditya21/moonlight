...
impl Module for HashMd5Module {
    ...
    fn run(&mut self, _ctx: &ModuleContext) -> Result<ModuleResult, ModuleError> {
        ...
        let expected = self
            ...
            .map(|o| o.value_as_string())
            .unwrap_or_default();
        let hash = md5::digest_hex(input.as_bytes());
...
pub struct HashMd5Factory;

impl ModuleFactory for HashMd5Factory {
    fn metadata(&self) -> &ModuleMetadata {
